...
def main():
    ...
    path    = []
    Q.append(start_node)
    G.nodes[start_node]['queued']  = True
        ...
        if begin_search and not reachGoal:
            if len(Q) > 0:
                ...
                if current_node == goal_node:
                    print('Goal!')
                    path        = backTrace(G, closed)
                    reachGoal   = True
                else:
                    neighbors       = G.neighbors(current_node)
                    for neighbor in neighbors:
                        if neighbor not in obs_list and not G.nodes[neighbor]['queued']:
                            x_dis2goal  = abs(int(neighbor[0])-int(goal_pos[0]))
                            y_dis2goal  = abs(int(neighbor[1])-int(goal_pos[1]))
                            h           = np.linalg.norm((x_dis2goal,y_dis2goal), 2)
                            x_dis2start = abs(int(neighbor[0])-int(start_pos[0]))
                            y_dis2start = abs(int(neighbor[1])-int(start_pos[1]))
                            g           = np.linalg.norm((x_dis2start,y_dis2start), 2)
                            f           = h+g
